...
<div class="row" ng-controller="projecttypesController" ng-init="manageProjectTypes()"> 
                ...
                <div class="row">
                    ...
                    <div class="col-sm-6 col-xs-12">
                        <div class="form-group">
                            <label for=""></label>
                            <span class="input-icon icon-right">
                                <a data-toggle="modal" data-target="#projecttypesModal" ng-click="initialModal(0, '', '')" class="btn btn-primary btn-right">Add Project Types</a>
                ...
                <table class="table table-hover table-striped table-bordered" at-config="config">
                    ...
                    <tbody>
                        <tr role="row" dir-paginate="list in ProjectTypesRow| filter:search |filter:searchData  | orderBy:orderByField:reverseSort | itemsPerPage:itemsPerPage">
                            ...
                            <td class="fa-div">
                                <div class="fa-hover" tooltip-html-unsafe="Edit project types" style="display: block;" data-toggle="modal" data-target="#projecttypesModal"><a href="javascript:void(0);" ng-click="initialModal({{ list.id}},'{{ list.project_type}}',{{ itemsPerPage}},{{$index}})"><i class="fa fa-pencil"></i></a></div>
                                <div class="fa-hover" tooltip-html-unsafe="Delete project types" style="display: block;" data-toggle="modal" data-target="#projecttypesModalD"><a href="javascript:void(0);" ng-click="initialModal({{ list.id}},'{{ list.project_type}}',{{ itemsPerPage}},{{$index}})"><i class="fa fa-trash-o"></i></a></div>
                            </td>
                        </tr>
                    </tbody>
                </table>
    ...
    <!-- Modal delete-->    
    <div class="modal fade" id="projecttypesModalD" role="dialog" tabindex="-1">    
        <div class="modal-dialog">
            <!-- Modal content-->
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal">&times;</button>
                    <h4 class="modal-title" align="center">Delete Project Types</h4>
                </div>
                <form novalidate ng-submit="projecttypesForm.$valid && doProjectTypesDelete()" name="projecttypesForm">
                    <input type="hidden" ng-model="csrfToken" name="csrftoken" id="csrftoken" ng-init="csrfToken = '<?php echo csrf_token(); ?>'" class="form-control">
                    <div class="modal-body">
                        <div class="form-group" ng-class="{ 'has-error' : sbtBtn && (!projecttypesForm.project_type.$dirty && projecttypesForm.project_type.$invalid)}">
                            <input type="hidden" class="form-control" ng-model="id" name="_id">
                    ...
                    <div class="modal-footer" align="center">
                        <button type="Submit" class="btn btn-primary" ng-click="sbtBtn = true" ng-disabled="proTypeBtn">Confirm</button>
                        <button type="button" class="btn btn-primary" ng-click="Cancel()" ng-disabled="proTypeBtn">Cancel</button>
                    </div> 
                </form>           
            </div>
        </div>
