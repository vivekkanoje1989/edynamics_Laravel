...
<div class="col-xs-12 col-md-12" ng-controller="taskPriorityCtrl" ng-init="getPriority(); vbreadcumbs = [
				{'displayName': 'Home', 'url': 'goDashboard()'},
				{'displayName': 'Settings', 'url': ''},
				{'displayName': 'List Management', 'url': ''},
				{'displayName': 'Manage Task Priority', 'url': ''}
			]">
	...
	<div class="widget">
		...
		<div class="widget-body">
			<div class="table-toolbar">
				<a id="editabledatatable_new" href="" class="btn btn-default" data-toggle="modal" data-target="#taskPriorityModal" ng-click="initTskPriority(0, '', '', '', '')">Add Priority</a>
					...
					<div class="DTTT btn-group">
						...
						<a class="btn btn-default DTTT_button_collection" id="ToolTables_editabledatatable_2">
							...
							<ul class="dropdown-menu dropdown-default">
								<li>
									<a href="javascript:void(0);" title="View print view" ng-click="ExportToxls()">Export</a>
								</li>
								<!--li>
                        ...
                        <b ng-repeat="(key, value) in searchData" ng-if="value != 0 && key != 'toDate'">
                            <div class="col-sm-2" data-toggle="tooltip" title="{{  key.substring(0, key.indexOf('_'))}}"> 
                                <div class="label graded fade in" style="margin-left: -15px;padding: 10px; margin-bottom: 10px; border: 1px solid #5db2ff; background-color: white;color:black;">
                                    ...
                                    <strong ng-if="key === 'priority_name'" data-toggle="tooltip" title="Priority Name"><strong> Priority Name : </strong> {{ value}}</strong>
                                </div>
                            </div>
                        </b>                        
                    </div>
				...
				<table class="table table-striped table-hover table-bordered dataTable no-footer" at-config="config">
					<thead >
						<tr role="row" style=" background-color: #eee; background-image: -moz-linear-gradient(top,#f2f2f2 0,#fafafa 100%); background-image: -o-linear-gradient(top,#f2f2f2 0,#fafafa 100%); background-image: linear-gradient(to bottom,#f2f2f2 0,#fafafa 100%); font-size: 12px;">
							...
							<th class="sorting" tabindex="0"  rowspan="1" colspan="1" style="width: 230px;" ng-click="OrderFunction()">Priority Name</th>
							<th  rowspan="1" colspan="1" aria-label="status" style="width: 170px;"></th>
						</tr>
					</thead>
					<tbody>
						<tr  role="row" dir-paginate="list in tskpriorityRow | itemsPerPage:itemsPerPage | filter:search | filter:searchData  | orderBy: OrderRec " >
							<td>{{ itemsPerPage * (noOfRows - 1) + $index + 1}}</td>							
                            <td>{{list.priority_name}}</td>							
							<td >
								<a href="#" class="btn btn-info btn-xs edit" tooltip-html-unsafe="Edit Priority" data-toggle="modal" data-target="#taskPriorityModal" ng-click='initTskPriority("{{ list.id}}","{{list.priority_name}}", "{{ itemsPerPage}}", "{{$index}}", " ")'><i class="fa fa-edit"></i> Edit</a>
								<a href="#" class="btn btn-danger btn-xs delete" tooltip-html-unsafe="Delete Priority"  data-toggle="modal" data-target="#taskPriorityModal" ng-click='initTskPriority("{{ list.id}}","{{list.priority_name}}", "{{ itemsPerPage}}", "{{$index}}", "del")'><i class="fa fa-trash-o"></i> Delete</a>
							</td>
						</tr>					
				</tbody>			
			</table>
			...
			<div class="DTTTFooter">
                <div class="col-sm-6">
                	<div class="dataTables_info" id="DataTables_Table_0_info" role="status" aria-live="polite" >Showing page {{noOfRows}} of {{ tskPriorityLength }} entries </div>
...
<!--model-->
<div class="modal fade" id="taskPriorityModal" role="dialog" tabindex="-1">    
        <div class="modal-dialog">
            ...
            <div class="modal-content">
                ...
                <form novalidate ng-submit="priorityForm.$valid && doPriorityAction()" method="post" name="priorityForm" enctype="multipart/form-data">
                    <input type="hidden" ng-model="csrfToken" name="csrftoken" id="csrftoken" ng-init="csrfToken = '<?php echo csrf_token(); ?>'" class="form-control">
                    <input type="hidden" ng-model="id" name="id" id="id" class="form-control">
                    <div class="modal-body">
                        <div class="row">
                            <div class="form-group col-sm-12" ng-class="{ 'has-error' : sbtBtn && (!priorityForm.priority_name.$dirty && priorityForm.priority_name.$invalid)}">
                                <label>Priority Name<span class="sp-err">*</span></label>
                                <span class="input-icon icon-right">                                    
                                    <input type="text" class="form-control" name="priority_name" ng-model="priority_name" >                                                                       
                                    <div class="help-block" ng-show="sbtBtn" ng-messages="priorityForm.priority_name.$error">
 ...
 <!-- Filter Form Start-->
    <div class="wrap-filter-form show-widget" id="slideout">
        <form name="priorityFilter" role="form" ng-submit="filterDetails(searchDetails)">
