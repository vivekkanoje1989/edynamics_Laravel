...
<div class="col-xs-12 col-md-12" ng-controller="taskStatusCtrl" ng-init="getStatus(); vbreadcumbs = [
				{'displayName': 'Home', 'url': 'goDashboard()'},
				{'displayName': 'Settings', 'url': ''},
				{'displayName': 'List Management', 'url': ''},
				{'displayName': 'Manage Task Status', 'url': ''}
			]">
	...
	<div class="widget">
		<div class="widget-header ">
			<span class="widget-caption">Manage Task Status</span>
		...
		<div class="widget-body">
			<div class="table-toolbar">
				<a id="editabledatatable_new" href="" class="btn btn-default" data-toggle="modal" data-target="#statusModal" ng-click="initStatus(0, '', '','','')">Add Status</a>
                        ...
                        <b ng-repeat="(key, value) in searchData" ng-if="value != 0 && key != 'toDate'">
                            <div class="col-sm-2" data-toggle="tooltip" title="{{  key.substring(0, key.indexOf('_'))}}"> 
                                <div class="label graded fade in" style="margin-left: -15px;padding: 10px; margin-bottom: 10px; border: 1px solid #5db2ff; background-color: white;color:black;">
                                    ...
                                    <strong ng-if="key === 'status_name'" data-toggle="tooltip" title="Status Name"><strong> Status Name : </strong> {{ value}}</strong>
                                </div>
                            </div>
                        </b>                        
                    </div>
				...
				<table class="table table-striped table-hover table-bordered dataTable no-footer" at-config="config">
					<thead >
						<tr role="row" style=" background-color: #eee; background-image: -moz-linear-gradient(top,#f2f2f2 0,#fafafa 100%); background-image: -o-linear-gradient(top,#f2f2f2 0,#fafafa 100%); background-image: linear-gradient(to bottom,#f2f2f2 0,#fafafa 100%); font-size: 12px;">
							...
							<th class="sorting" tabindex="0"  rowspan="1" colspan="1" style="width: 230px;" ng-click="OrderFunction()">Status Name</th>
							<th  rowspan="1" colspan="1" aria-label="status" style="width: 170px;"></th>
						</tr>
					</thead>
					<tbody>
						<tr  role="row" dir-paginate="list in statusRow | itemsPerPage:itemsPerPage | filter:search | filter:searchData  | orderBy: OrderRec " >
							<td>{{ itemsPerPage * (noOfRows - 1) + $index + 1}}</td>							
                            <td>{{list.status_name}}</td>							
							<td >
								<a href="#" class="btn btn-info btn-xs edit" tooltip-html-unsafe="Edit Status" data-toggle="modal" data-target="#statusModal" ng-click='initStatus("{{ list.id}}","{{list.status_name}}", "{{ itemsPerPage}}", "{{$index}}", " ")'><i class="fa fa-edit"></i> Edit</a>
								<a href="#" class="btn btn-danger btn-xs delete" tooltip-html-unsafe="Delete Status"  data-toggle="modal" data-target="#statusModal" ng-click='initStatus("{{ list.id}}","{{list.status_name}}", "{{ itemsPerPage}}", "{{$index}}", "del")'><i class="fa fa-trash-o"></i> Delete</a>
							</td>
						</tr>					
				</tbody>			
			</table>
			...
			<div class="DTTTFooter">
                <div class="col-sm-6">
                	<div class="dataTables_info" id="DataTables_Table_0_info" role="status" aria-live="polite" >Showing page {{noOfRows}} of {{ statusLength }} entries </div>
...
<div class="modal fade" id="statusModal" role="dialog" tabindex="-1">    
        <div class="modal-dialog">
            ...
            <div class="modal-content">
                ...
                <form novalidate ng-submit="statusForm.$valid && doStatusAction()" method="post" name="statusForm" enctype="multipart/form-data">
                    <input type="hidden" ng-model="csrfToken" name="csrftoken" id="csrftoken" ng-init="csrfToken = '<?php echo csrf_token(); ?>'" class="form-control">
                    <input type="hidden" ng-model="id" name="id" id="id" class="form-control">
                    <div class="modal-body">
                        <div class="row">
                            <div class="form-group col-sm-12" ng-class="{ 'has-error' : sbtBtn && (!statusForm.status_name.$dirty && statusForm.status_name.$invalid)}">
                                <label>Priority Name<span class="sp-err">*</span></label>
                                <span class="input-icon icon-right">                                    
                                    <input type="text" class="form-control" name="status_name" ng-model="status_name" >                                                                       
                                    <div class="help-block" ng-show="sbtBtn" ng-messages="statusForm.status_name.$error">
        ...
        <div class="modal-dialog">           
            <div class="modal-content" style="border: 3px solid azure;border-radius: 30px;height: 489px; background: #0e0e0e38;overflow: auto;">
                <div class="modal-header widget-header bordered-bottom bordered-themeprimary" style="border-radius: 27px; margin-top: 25px; width: 90%;margin-left: 20px;margin-right: 20px;">
                    ...
                    <h4 class="modal-title" align="center">Task Status Help Info</h4>
                </div>                
                <div class="modal-body" style="">
                    <div class="form-group ">
						<div class="form-group col-sm-3">
							<label style="font-family: serif;font-size: 17px;">Status <span class="sp-err"> : </span></label>
						</div>
						<div class="form-group col-sm-9">
							<p style="font-family: serif;font-size: 17px;">The Status is defined here and this is being use as Task Status in Task Management.<p>
                        </div>				
                    </div> 
					<div class="form-group ">
						<div class="form-group col-sm-3">
            ...
            <div class="row">
                <div class="col-sm-12 col-xs-12">
                    <div class="form-group">
                        ...
                        <span class="input-icon icon-right">
                            <input type="text" ng-model="searchDetails.status_name" name="status_name" class="form-control">
                        </span>
                    </div>
                </div>
            </div>
