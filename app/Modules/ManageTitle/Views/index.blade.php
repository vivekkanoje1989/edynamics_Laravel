...
<div class="col-xs-12 col-md-12" ng-controller="titleController" ng-init="manageTitle();vbreadcumbs = [
				{'displayName': 'Home', 'url': 'goDashboard()'},
				{'displayName': 'Settings', 'url': ''},
				{'displayName': 'List Management', 'url': ''},
				{'displayName': 'Manage Titles', 'url': ''}
			]">
	...
	<div class="widget">
		<div class="widget-header ">
			<span class="widget-caption">Manage Titles</span>
		...
		<div class="widget-body">
			<div class="table-toolbar">
				<a id="editabledatatable_new" href="" class="btn btn-default" data-toggle="modal" data-target="#titleModal" ng-click="initialModal(0, '', '', '', '','')">{{adnBtn}}</a>
                        ...
                        <b ng-repeat="(key, value) in searchData" ng-if="value != 0 && key != 'toDate'">
                            <div class="col-sm-2" data-toggle="tooltip" title="{{  key.substring(0, key.indexOf('_'))}}"> 
                                <div class="label graded fade in" style="margin-left: -15px;padding: 10px; margin-bottom: 10px; border: 1px solid #5db2ff; background-color: white;color:black;">
                                    ...
                                    <strong ng-if="key === 'title'" data-toggle="tooltip" title="Profession"><strong> Title : </strong> {{ value}}</strong>
				...
				<table class="table table-striped table-hover table-bordered dataTable no-footer" at-config="config">
					<thead >
						<tr role="row" style=" background-color: #eee; background-image: -moz-linear-gradient(top,#f2f2f2 0,#fafafa 100%); background-image: -o-linear-gradient(top,#f2f2f2 0,#fafafa 100%); background-image: linear-gradient(to bottom,#f2f2f2 0,#fafafa 100%); font-size: 12px;">
							...
							<th class="sorting" tabindex="0"  rowspan="1" colspan="1" style="width: 259px;" ng-click="OrderFunction('Title')">Title Name</th>
					...
					<tbody>
						<tr  role="row" dir-paginate="list in titleRow | itemsPerPage:itemsPerPage | filter:search | filter:searchData  | orderBy: OrderRec " >
							<td>{{ itemsPerPage * (noOfRows - 1) + $index + 1}}</td>							
                            <td>{{ list.title}}</td> 
                            <td>{{ list.status == 1 ? "Active" : "Inactive" }}</td> 
                            <td >
								<a href="#" class="btn btn-info btn-xs edit" tooltip-html-unsafe="Edit Title" data-toggle="modal" data-target="#titleModal" ng-click='initialModal({{ list.id}},"{{list.title}}","{{list.status}}",{{ itemsPerPage}},{{$index}}," ")'><i class="fa fa-edit"></i> Edit</a>
								<a href="#" class="btn btn-danger btn-xs delete" tooltip-html-unsafe="Delete Title"  data-toggle="modal" data-target="#titleModal" ng-click='initialModal("{{ list.id}}","{{list.title}}","{{list.status}}",{{ itemsPerPage}},{{$index}},"del")'><i class="fa fa-trash-o"></i> Delete</a>
							</td>
						</tr>					
				</tbody>			
			</table>
			...
			<div class="DTTTFooter">
                <div class="col-sm-6">
                	<div class="dataTables_info" id="DataTables_Table_0_info" role="status" aria-live="polite">Showing page {{noOfRows}} of {{ titleRowLength }} entries</div>
...
<!--model-->
    <div class="modal fade" id="titleModal" role="dialog" tabindex="-1">   
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form novalidate ng-submit="titleForm.$valid && doTitleAction()" name="titleForm">
                    <input type="hidden" ng-model="csrfToken" name="csrftoken" id="csrftoken" ng-init="csrfToken = '<?php echo csrf_token(); ?>'" class="form-control">

                    <div class="modal-body">
                        <div class="form-group" ng-class="{ 'has-error' : sbtBtn && (!titleForm.title.$dirty && titleForm.title.$invalid)}">
                            <input type="hidden" class="form-control" ng-model="id" name="id">
                            <label>Title name<span class="sp-err">*</span></label>
                            <span class="input-icon icon-right">
                                <input type="text" class="form-control" ng-model="title" name="title"  ng-change="errorMsg = null" required>

                                <div class="help-block" ng-show="sbtBtn" ng-messages="titleForm.title.$error">
                                    <div ng-message="required">Title is required</div>
                                    <div ng-if="errorMsg" class="err">{{errorMsg}}</div>
                                </div>
                            </span>
                        </div>
                        <div class="form-group" ng-class="{ 'has-error' : sbtBtn && (!titleForm.status.$dirty && titleForm.status.$invalid)}">
                            <input type="hidden" class="form-control" ng-model="id" name="id">
                            ...
                            <span class="input-icon icon-right">
                                ...
                                <div class="help-block" ng-show="sbtBtn" ng-messages="titleForm.status.$error">
                    ...
                    <div class="modal-footer" align="center">
                        <button type="Submit" class="btn btn-sub" ng-click="sbtBtn = true" ng-disabled="titleBtn">{{action}}</button>
        ...
        <div class="modal-dialog">           
            <div class="modal-content" style="border: 3px solid azure;border-radius: 30px;height: 489px; background: #0e0e0e38;overflow: auto;">
                <div class="modal-header modal-header widget-header bordered-bottom bordered-themeprimary" style="border-radius: 27px; margin-top: 25px; width: 90%;margin-left: 20px;margin-right: 20px;">
                    ...
                    <h4 class="modal-title" align="center">Manage Title Help Info</h4>
                </div>                
                <div class="modal-body" style="">
                    <div class="form-group ">
						<div class="form-group col-sm-3">
							<label style="font-family: serif;font-size: 17px;">Title <span class="sp-err"> : </span></label>
						</div>
						<div class="form-group col-sm-9">
							<p style="font-family: serif;font-size: 17px;">The Title is defined here and this is being use in various forms.<p>
                        </div>				
                    </div> 
					<div class="form-group ">
						<div class="form-group col-sm-3">
            ...
            <div class="row">
                <div class="col-sm-12 col-xs-12">
                    <div class="form-group">
                        ...
                        <span class="input-icon icon-right">
                            <input type="text" ng-model="searchDetails.title" name="title" class="form-control">
                        </span>
                    </div>
